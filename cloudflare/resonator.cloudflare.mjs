...
const LIMIT = 30000;		// Header size limit in bytes (default: 30000)
	...
	async fetch(request, env, ctx) {
		...
		if (url.pathname === "/rhythm/archive" && url.searchParams.get("token") === ARCHIVING.STORE.TOKEN) {
			...
			for (const target of targets) {
				...
				const [list1, list2] = await Promise.all([ // Scan folders in parallel
					list(env[ARCHIVING.STORE.NAME], day1 + '/'),
				...
				if (ARCHIVING.NDJSON) { // NDJSON: single-line JSON only, no metadata or AI insights
					const lines = [];
					if (objects.length) {
						for (let b = 0; b < objects.length; b += ARCHIVING.STORE.BATCH) { // Files per batch for memory safety (default: 6)
							const batch = objects.slice(b, b + ARCHIVING.STORE.BATCH);
							const files = await Promise.all(batch.map(object => env[ARCHIVING.STORE.NAME].get(object.key)));
							...
							for (const text of texts) {
								for (const line of text.split('\n').filter(Boolean)) {
									try { // JSON parse error protection
										...
										const time = parsed.Logs[0].TimestampMs;
										if (time < start || time >= end) continue; // Skip if not target date
										if (message[0] !== '{') continue; // Skip non-JSON logs like bot detection
					...
					if (objects.length) { // Handle existing logs
							...
							for (const text of texts) {
								for (const line of text.split('\n').filter(Boolean)) {
									try { // JSON parse error protection
										...
										const time = parsed.Logs[0].TimestampMs;
										if (time < start || time >= end) continue; // Skip if not target date										
										if (message[0] !== BEAT.SRF.M) continue; // Skip non-BEAT logs like bot detection
										const split = message.indexOf('\n');
						...
						if (meta.total > 0) { // Calculate averages after all batches complete
...
// List all R2 objects with pagination
async function list(r2, prefix) {
	const objects = [];
	let cursor;
	do {
		const result = await r2.list(cursor ? {prefix, cursor} : {prefix});
