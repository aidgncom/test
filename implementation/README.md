...
BEAT can also be handled natively on AI Accelerators (xPU). BEAT's 1-byte scan performance makes it inherently optimized for massive parallel handling and large-scale AI training. Below is an example Triton kernel that encodes BEAT tokens directly in xPU memory.
